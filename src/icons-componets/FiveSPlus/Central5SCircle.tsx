import React from "react";
import { useLanguage } from "../../contexts/LanguageContext";
import { Language } from "../../types";

const fiveSStepsByLanguage: Record<
  Language,
  { step: string; title: string; color: string }[]
> = {
  es: [
    { step: "1- SEIRI", title: "ORGANIZAR", color: "bg-red-500" },
    {
      step: "2- SEITON",
      title: "ORDENAR",
      color: "bg-[var(--color-secondary)]",
    },
    { step: "3- SEISO", title: "LIMPIAR", color: "bg-purple-600" },
    { step: "4- SEIKETSU", title: "ESTANDARIZAR", color: "bg-blue-500" },
    { step: "5- SHITSUKE", title: "MANTENER", color: "bg-orange-500" },
  ],
  en: [
    { step: "1- SEIRI", title: "SORT", color: "bg-red-500" },
    {
      step: "2- SEITON",
      title: "SET IN ORDER",
      color: "bg-[var(--color-secondary)]",
    },
    { step: "3- SEISO", title: "SHINE", color: "bg-purple-600" },
    { step: "4- SEIKETSU", title: "STANDARDIZE", color: "bg-blue-500" },
    { step: "5- SHITSUKE", title: "SUSTAIN", color: "bg-orange-500" },
  ],
  pt: [
    { step: "1- SEIRI", title: "ORGANIZAR", color: "bg-red-500" },
    {
      step: "2- SEITON",
      title: "ORDENAR",
      color: "bg-[var(--color-secondary)]",
    },
    { step: "3- SEISO", title: "LIMPAR", color: "bg-purple-600" },
    { step: "4- SEIKETSU", title: "PADRONIZAR", color: "bg-blue-500" },
    { step: "5- SHITSUKE", title: "MANTER", color: "bg-orange-500" },
  ],
};

const Central5SCircle: React.FC = () => {
  const { currentLanguage } = useLanguage();
  const lang = (currentLanguage || "es") as Language;
  const fiveSSteps = fiveSStepsByLanguage[lang];

  return (
    <div className="relative flex items-center justify-center h-80">
      {/* Central 5S Circle */}
      <div className="w-32 h-32 bg-gradient-to-br from-[var(--color-secondary)] to-[var(--color-accent)] rounded-full flex items-center justify-center text-white font-bold text-3xl z-10 shadow-xl">
        5S
      </div>

      {/* Surrounding Steps */}
      {fiveSSteps.map((step, index) => {
        const angle = index * 72 - 90; // 360/5 = 72 degrees between each step, start from top
        const radius = 120;
        const x = Math.cos((angle * Math.PI) / 180) * radius;
        const y = Math.sin((angle * Math.PI) / 180) * radius;

        return (
          <div
            key={index}
            className={`absolute w-24 h-24 ${step.color} rounded-full flex flex-col items-center justify-center text-white text-xs font-bold transform -translate-x-1/2 -translate-y-1/2 shadow-lg hover:scale-110 transition-transform duration-300`}
            style={{
              left: `calc(50% + ${x}px)`,
              top: `calc(50% + ${y}px)`,
            }}
          >
            <div className="text-[10px] leading-tight text-center">
              <div className="font-bold">{step.step}</div>
              <div className="mt-1">{step.title}</div>
            </div>
          </div>
        );
      })}
    </div>
  );
};

export default Central5SCircle;
